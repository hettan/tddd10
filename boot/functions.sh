...
echo "DDDDDD" $KERNEL_OPTIONS
    xterm -T kernel -e bash -c "java -Xmx512m -cp $CP kernel.StartKernel $KERNEL_OPTIONS 2>&1 | tee $LOGDIR/kernel-out.log" &
    PIDS="$PIDS $!"
...
function startSims {
    ...
    # PIDS="$PIDS $!"
    # Simulators
    xterm -T misc -e bash -c "java -Xmx512m -cp $CP:$BASEDIR/jars/rescuecore2.jar:$BASEDIR/jars/standard.jar:$BASEDIR/jars/misc.jar rescuecore2.LaunchComponents misc.MiscSimulator -c $CONFIGDIR/misc.cfg $* 2>&1 | tee $LOGDIR/misc-out.log" &
    PIDS="$PIDS $!"
echo "DDDDDD" $CONFIGDIR
    if [ "$HAVE_TRAFFIC_SIM" -eq "1" ]; then
      xterm -T collapse -e bash -c "java -Xmx512m -cp $CP:$BASEDIR/jars/rescuecore2.jar:$BASEDIR/jars/standard.jar:$BASEDIR/jars/collapse.jar rescuecore2.LaunchComponents collapse.CollapseSimulator -c $CONFIGDIR/collapse.cfg $* 2>&1 | tee $LOGDIR/collapse-out.log" &
      PIDS="$PIDS $!"
      xterm -T clear -e bash -c "java -Xmx512m -cp $CP:$BASEDIR/jars/rescuecore2.jar:$BASEDIR/jars/standard.jar:$BASEDIR/jars/clear.jar rescuecore2.LaunchComponents clear.ClearSimulator -c $CONFIGDIR/clear.cfg $* 2>&1 | tee $LOGDIR/clear-out.log" &
      PIDS="$PIDS $!"
    fi
    if [ "$HAVE_FIRE_SIM" -eq "1" ]; then
      xterm -T fire -e bash -c "java -Xmx512m -cp $CP:$BASEDIR/jars/rescuecore2.jar:$BASEDIR/jars/standard.jar:$BASEDIR/jars/resq-fire.jar:$BASEDIR/oldsims/firesimulator/lib/commons-logging-1.1.1.jar rescuecore2.LaunchComponents firesimulator.FireSimulatorWrapper -c $CONFIGDIR/resq-fire.cfg $* 2>&1 | tee $LOGDIR/fire-out.log" &
      PIDS="$PIDS $!"
      xterm -T ignition -e bash -c "java -Xmx512m -cp $CP:$BASEDIR/jars/rescuecore2.jar:$BASEDIR/jars/standard.jar:$BASEDIR/jars/ignition.jar rescuecore2.LaunchComponents ignition.IgnitionSimulator -c $CONFIGDIR/ignition.cfg $* 2>&1 | tee $LOGDIR/ignition-out.log" &
      PIDS="$PIDS $!"
    fi

      xterm -T traffic -e bash -c "java -Xmx512m -cp $CP:$BASEDIR/jars/rescuecore2.jar:$BASEDIR/jars/standard.jar:$BASEDIR/jars/traffic3.jar rescuecore2.LaunchComponents traffic3.simulator.TrafficSimulator -c $CONFIGDIR/traffic3.cfg $* 2>&1 | tee $LOGDIR/traffic-out.log" &
      PIDS="$PIDS $!"
...
#     
    ...
    if [ "$HAVE_AMBULANCE_SIM" -eq "1" ]; then
        if [ "$USE_STATIC_CIVILIANS" -eq "1" ]; then
                xterm -T civilian -e bash -c "java -Xmx1024m -cp $CP:$BASEDIR/jars/rescuecore2.jar:$BASEDIR/jars/standard.jar:$BASEDIR/jars/kernel.jar:$BASEDIR/extensions/staticcivilians.jar rescuecore2.LaunchComponents StaticCivilians.SampleCivilian*n -c $CONFIGDIR/civilian.cfg $* 2>&1 | tee $LOGDIR/civilian-out.log" &
                PIDS="$PIDS $!"
        else
                xterm -T civilian -e bash -c "java -Xmx1024m -cp $CP:$BASEDIR/jars/rescuecore2.jar:$BASEDIR/jars/standard.jar:$BASEDIR/jars/sample.jar:$BASEDIR/jars/kernel.jar rescuecore2.LaunchComponents sample.SampleCivilian*n -c $CONFIGDIR/civilian.cfg $* 2>&1 | tee $LOGDIR/civilian-out.log" &
                PIDS="$PIDS $!"
        fi;
    fi;
sleep 2
    ...
    if [ "$HAVE_VIEWER" -eq "1" ]; then
        xterm -T viewer -e bash -c "java -Xmx512m -cp $CP:$BASEDIR/jars/rescuecore2.jar:$BASEDIR/jars/standard.jar:$BASEDIR/jars/sample.jar rescuecore2.LaunchComponents sample.SampleViewer -c $CONFIGDIR/viewer.cfg $TEAM_NAME_ARG $* 2>&1 | tee $LOGDIR/viewer-out.log" &
        PIDS="$PIDS $!"
